...
class DictionaryIterator(keras.preprocessing.image.Iterator):
    ...
    def _get_batches_of_transformed_samples(self, selection):
        ...
        for batch_index, image_index in enumerate(selection):
            ...
            # print("_get_batches_of_transformed_samples 3", batch_index, image_index)

            # while count == 0:
            # print("_get_batches_of_transformed_samples 3.1", count)

            # Image
            target_image_pathname = self.dictionary[image_index]["image"]["pathname"]

            target_image = skimage.io.imread(target_image_pathname)
            # print("target_image 1", target_image.shape)

            if target_image.ndim == 2:
                target_image = numpy.expand_dims(target_image, -1)
                # print("target_image 2", target_image.shape)
            ...
            # print("target_image 3", target_image.shape)
            ...
            for object_index, b in enumerate(self.dictionary[image_index]["objects"]):
                ...
                # print("_get_batches_of_transformed_samples 3.5", image_index, len(self.dictionary[image_index]["objects"]))

                if b["class"] not in self.classes:
                    continue

                # print("_get_batches_of_transformed_samples 3.6", object_index, b)
                ...
                # print("_get_batches_of_transformed_samples 3.8", minimum_c, maximum_c, target_image.shape[1], minimum_r, maximum_r, target_image.shape[0])
